...
function Main() {
      ...
      if (doesHaveViewPermission || role === SUPER_ADMIN) {
        await dispatch(
          fetchAllCompaniesData({
            ...
            searchText,
            ...(filterCompany !== "all" && {
              enable: filterCompany === "active" ? 1 : 0,
            }),
          })
        );
      } else {
  ...
  const resetFilter = async () => {
    ...
    try {
      ...
      await dispatch(fetchAllCompaniesData({ limit: PAGE_LIMIT, page: 1 }));
    } catch (error) {
      console.log("Err--", error);
    } finally {
  ...
  const activateSelectedCompany = async () => {
    try {
      setIsLoading(true);
      const res = await dispatch(
        changeCompanyStatus({
          enable: isCompanyChecked ? 1 : 0,
  ...
  const deleteSelectedCompany = async () => {
    try {
      setIsLoading(true);
      const res = await dispatch(deleteCompany(Number(selectedCompanyId)));
      if (res.payload?.status === SUCCESS_CODE) {
        dispatch(fetchAllCompaniesData({ limit, page: 1, searchText: "" }));
        toast.success(
          res.payload.data.message || "Company deleted successfully"
        );
        setFilterCompany("all");
        setSearchText("");
        setCurrentPage(1);
  ...
  const updateCompany = (id: number) => {
    secureLocalStorage.setItem("newlyAddedCompany", String(id));
  ...
  return (
    <>
      {(hasViewPermission || role === SUPER_ADMIN) && (
        <div>
          <div
            className={`flex px-2 flex-wrap gap-3 sm:gap-5 mt-3 sm:mt-5 ${
              role !== SUPER_ADMIN ? "justify-end" : "justify-between"
            }`}
          >
            {role === SUPER_ADMIN && (
                ...
                <Controller
                  ...
                  render={({ field: { value } }) => {
                    ...
                    const selectedVal = filterList?.find(
                      (option) => option.value === filterCompany
                    );
                    const defaultValue =
            ...
            {companiesState.loading ? (
              ...
              <>
                            ...
                            {role === SUPER_ADMIN && (
                              <Table.Td className="first:rounded-l-md last:rounded-r-md bg-white border border-r-0 border-l-0 first:border-l last:border-r border-slate-200 dark:bg-darkmode-600 dark:border-darkmode-600 shadow-[20px_3px_20px_#0000000b] py-0 relative before:block before:w-px before:h-8 before:bg-slate-200 before:absolute before:left-0 before:inset-y-0 before:my-auto before:dark:bg-darkmode-400">
                                <div className="flex items-center">
                                  <FormSwitch>
                                    <FormSwitch.Input
                                      id={`checkbox-switch-7${company.id}`}
                                      type="checkbox"
                                      className="shadow-xl border-slate-300"
                                      onChange={(
                                        e: ChangeEvent<HTMLInputElement>
                                      ) => changeStatus(e, company.id)}
                ...
                <CustomPagination
                  ...
                  totalRecords={companiesState.totalRecords}
                />
                {/* )} */}
                {/* END: Pagination */}
              </>
          ...
          <ActivateModal
            ...
            onClose={() => setActivateCompanyModal(false)}
            onClick={activateSelectedCompany}
