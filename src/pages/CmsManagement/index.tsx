...
const Index = () => {
  ...
  const { cmsData, totalPages, loading, error, totalRecords } = useSelector(
  ...
  const updateCms = (id: number) => {
    console.log("Update CMS with ID:", id);
  };
  ...
  const updateCmsbyid = async (id: number) => {
    try {
      navigate("/cms/addDetails", { state: { cmsId: id } });
    } catch (err) {
      ...
      <div>
        <div className="flex px-2 flex-wrap gap-5 justify-between mt-5">
          ...
          <Button
            ...
            onClick={() => navigate("/cms/addDetails")}
          >
        ...
        >
          <DialogContent>
            ...
            <div className="px-5 pb-8 text-center">
              <Button variant="danger" onClick={handleDeleteCms}>
                {isLoading && (
